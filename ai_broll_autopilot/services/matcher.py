...
class Matcher:
    ...
    async def _resolve_single_shot(self, shot: Dict[str, Any], job_cache_dir: Path) -> Optional[Dict[str, Any]]:
        ...
        # 2. Try Pexels royalty-free vertical footage first if API key is provided
        ...
        # 3. If not cached or resolved from Pexels, acquire asset based on style
        if not asset_path:
            if style == "collage":
        ...
        # 4. If stockpile acquisition failed, automatically fallback to procedural collage
